...
// 視窗 < md (768px) 視為行動裝置：此時側欄為覆蓋式抽屜，選擇對話後自動關閉。
const isMobile = () =>
...
export default function Sidebar({ open, onClose }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex h-full w-72 max-w-[80vw] shrink-0 transform flex-col border-r border-ink-700 bg-ink-850 transition-transform duration-200 md:w-64 md:max-w-none md:transition-none ${
          open
          ...
          {conversations.map((c) => (
            ...
            >
              ...
              <button
                ...
                className="shrink-0 text-gray-500 opacity-0 hover:text-red-400 group-hover:opacity-100 max-md:opacity-100"
                title={t("deleteChat")}
              >
                <Trash2 size={15} />
              </button>
